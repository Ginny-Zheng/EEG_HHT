...
plot(try$hinstfreq[, 1]~try$tt, ylim = c(0, 45), col = rainbow(128)[try$hamp[, 1]])

plot(hgram$hinstfreq[, 1,1]~try$tt, ylim = c(0, 45))
...
### calculate frequency ratio feature
...
matrix(wave_all[[3]][22, ], byrow = TRUE, 4,4)
...
round(wave_all[[2]][i,1:4]*100,2)
round(wave_all[[3]][i,1:4]*100, 2)

round(wave_amp[[2]][i,1:4], 2)
round(wave_amp[[3]][i,1:4], 2)
...
############ p18c4
load("p18c4data.RData")
load("wave_result/p18c4_wave_ratio.RData")
